...
class getMetric(nn.Module):
	...
	def forward(self, doa_gt, vad_gt, doa_est, vad_est, ae_mode, ae_TH=30, ae_TH_mae = 180, useVAD=True, vad_TH=[0.5,0.5]):
		"""
		Args:
		...
		"""
  		...
  		# ae_TH = 10
		# vad_TH = [0.001, 0.5]
		# doa_gt, doa_est [1,23,2,2] [nb,nt,nele/nazi,track]
		# vad_gt, vad_est [1,23,2] [nb,nt,track]
		...
		if self.source_mode == 'single':
			...
			for b_idx in range(nbatch):
				...
				corr_flag = torch.zeros((nt, num_sources_gt)).to(device)  # tolerance가 10 or 15인 나머지 measure에 적용
				corr_flag_mae = torch.zeros((nt, num_sources_gt)).to(device)  # tolerance가 180인 MAE,RMSE measure에 적용
				azi_error = torch.zeros((nt, num_sources_gt)).to(device)
				azi_error_mae = torch.zeros((nt, num_sources_gt)).to(device)
				...
				vad_est_one = vad_est_one * vad_gt_sum  # GT를 참고해 소스가 확실히 존재하는 프레임만 보기 위해 곱해줌
					...
					if num_gt>0 and num_est>0:
						...
						for gt_idx in range(num_gt):
							for est_idx in range(num_est):
								dist_mat_az[gt_idx, est_idx] = self.angular_error(est[1,est_idx], gt[1,gt_idx], 'azi')  # 모든 gt-pred쌍에 대한 azimuth error matrix를 구함
						...
						invalid_assigns = dist_mat_az > ae_TH  # tolerance가 10 or 15인 나머지 measure에 적용
						invalid_assigns_mae = dist_mat_az > ae_TH_mae  # tolerance가 180인 MAE,RMSE measure에 적용
						
						# tolerance가 10 or 15인 나머지 measure에 적용
						dist_mat_az_bak = dist_mat_az.clone()  # deep copy
						dist_mat_az_bak[invalid_assigns] = self.inf  # error tolerance 넘어가는 값들은 self.inf=10000으로 설정
						assignment = list(linear_sum_assignment(dist_mat_az_bak))  # 헝가리안 알고리즘 - gt와 pred 값 간 azimuth error를 최소화하는 최적 쌍을 매칭
						assignment = self.judge_assignment(dist_mat_az_bak, assignment)  # 매칭된 쌍의 error가 self.inf가 아니면 유효한 매칭으로 간주, 무한대면 무효 매칭 기본값을 self.invlid_sidx=10으로 설정

						# tolerance가 180인 MAE,RMSE measure에 적용
						dist_mat_az_bak_mae = dist_mat_az.clone()  # deep copy
						dist_mat_az_bak_mae[invalid_assigns_mae] = self.inf
						assignment_mae = list(linear_sum_assignment(dist_mat_az_bak_mae))
						assignment_mae = self.judge_assignment(dist_mat_az_bak_mae, assignment_mae)
						...
						for src_idx in range(num_gt):
							...
							if assignment[src_idx] != self.invlid_sidx:
								...
								azi_error[t_idx, src_idx] = dist_mat_az[src_idx, assignment[src_idx]]  # 최적 매칭 후 소스 정렬
							...
							# tolerance가 180인 MAE,RMSE measure에 적용
							if assignment_mae[src_idx] != self.invlid_sidx:
								corr_flag_mae[t_idx, src_idx] = 1
								azi_error_mae[t_idx, src_idx] = dist_mat_az[src_idx, assignment_mae[src_idx]]  # 어차피 azimuth error만 구할건데 뭐..
       
        
				K_corr = corr_flag.sum(axis=1)
				K_corr_mae = corr_flag_mae.sum(axis=1)
				...
				if 'ele' in ae_mode:
					...
					mae_temp += [((azi_error_mae*corr_flag_mae).sum(axis=0)).sum() / (K_corr_mae.sum(axis=0)+1e-5)]
					rmse_temp += [torch.sqrt(((azi_error_mae*azi_error_mae*corr_flag_mae).sum(axis=0)).sum() / (K_corr_mae.sum(axis=0)+1e-5))]
				elif 'aziele' in ae_mode:
...
class PredDOA(nn.Module):
	...
	def forward(self,pred_batch, gt_batch,idx):
		...
		return metric
	def pred2DOA(self, pred_batch, gt_batch, mask=None):
		"""
		...
		for i in range(self.max_track):
			...
			if mask is not None:
				pred_batch_temp, gt_batch_temp = self.pred2DOA_track(pred_ipd[:,:,:,i], gt_batch, mask=mask)
			else:
				pred_batch_temp, gt_batch_temp = self.pred2DOA_track(pred_ipd[:,:,:,i], gt_batch)
	...
	def pred2DOA_track(self, pred_batch=None, gt_batch=None, time_pool_size=None, mask=None):
		"""
		Convert Estimated IPD of one track to DOA
	    """

		if pred_batch is not None:
      		# 여기서 pred_batch는 source track별 pred_ipd 
			...
			nbin = (nmic*nf/2)
			if mask is not None:
				mask_rebatch = self.removebatch(mask, nb).permute(0, 2, 3, 1)
				nbin = nmic * mask_rebatch.contiguous().view(nb, nt, -1).sum(2, keepdim=True) / 2
				nbin = nbin.to(self.dev)
			pred_spatial_spectrum = torch.bmm(pred_rebatch.contiguous().view(nb, nt, -1),
											  dpipd_template.contiguous().view(nb, nele, nazi, -1).permute(0, 3, 1, 2).view(nb, nmic * nf, -1))/nbin  # (nb, nt, nele*nazi)
			# pred_spatial_spectrum는 [1,23,512]와 [1,512,180]를 내적하여 [1,23,180]를 반환함
			...
			for source_idx in range(self.max_num_sources):
				...
				for b_idx in range(nb):
					for t_idx in range(nt):
						...
						if mask is not None:
							max_dpipd_template[b_idx, t_idx, :, :] = \
								max_dpipd_template[b_idx, t_idx, :, :] * mask[b_idx, t_idx, :, None].cuda()
						# ratio: DP-IPD template과 모델 예측값을 내적하여 예측값과 소스가 있는 방향이 얼마나 유사한지 score를 계산
						# ratio 분모: pred_rebatch랑 max_dpipd_template 간 correlation
						# ratio 분자: max_dpipd_template의 autocorrelation
						ratio = torch.sum(
				...
				pred_rebatch = pred_rebatch - max_dpipd_template  # 강한 소스 신호를 제거해 남은 소스 신호 내에서 다시 계산, 최대 소스 개수만큼 반복
	...
	def evaluate(self, pred_batch=None, gt_batch=None, vad_TH=[0.001, 0.5],idx=None):
		...
		metric['ACC'], metric['MDR'], metric['FAR'], metric['MAE'], metric['RMSE'] = \
			self.getmetric(doa_gt, vad_gt, doa_est, vad_est, ae_mode = ae_mode, ae_TH=10, ae_TH_mae=10, useVAD=True, vad_TH=vad_TH)  

		# results = []
		# vad_TH_list = np.linspace(0.001, 0.1, 100)
		# for i in vad_TH_list:
		# 	vad_TH = [0.001, i]
		# 	metric['ACC'], metric['MDR'], metric['FAR'], metric['MAE'], metric['RMSE'] = \
		# 		self.getmetric(doa_gt, vad_gt, doa_est, vad_est, ae_mode = ae_mode, ae_TH=10, useVAD=True, vad_TH=vad_TH)
		# 	results.append((i, metric['ACC'].item(), metric['MAE'].item()))
		# 	# print(f'pred_vad_TH:{i}', metric)
		# 	# print()
		# best_vad_TH, best_ACC, best_MAE = max(results, key=lambda x:x[1])
		# print(f'best_vad_TH:{best_vad_TH}, best_ACC:{best_ACC}, best_MAE:{best_MAE}')
		# print()
