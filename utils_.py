...
def save_file(mic_signal, acoustic_scene, sig_path, acous_path):
    ...
    if acous_path is not None:
        acous_dict = acoustic_scene.__dict__
        acous_keys = list(acous_dict.keys())
        for k in acous_keys:
            if k not in ['array_setup','dp_mic_signals_sources', 'DOA', 'source_vad', 'mic_vad_sources']:
                acous_dict.pop(k)
        file = open(acous_path, 'wb')
        file.write(pickle.dumps(acous_dict))
        file.close()
...
def load_file(acoustic_scene, sig_path, acous_path):
    ...
    if acous_path is not None:
        ...
        ### npz compressed 새로 짠 코드 
        data = np.load(acous_path, allow_pickle=True)
        acoustic_scene.__dict__ = data['arr_0'].item()
