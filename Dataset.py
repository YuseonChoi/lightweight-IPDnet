...
class FixTrajectoryDataset(Dataset):
	def __init__(self, data_dir, dataset_sz, transforms=None, return_acoustic_scene=False, return_fname=False):
	...
	def __getitem__(self, idx):
		...
		if self.return_acoustic_scene:
			if self.return_fname:
				return mic_signals, acoustic_scene, os.path.basename(sig_path)
			else:
			...
			# 원래 dp_mic_signals_sources의 경우 마지막 차원이 소스가 한 개면 1, 두 개면 2였음
			# dp_signal은 차원이 (72000,2,2)로 고정돼있음
			if gts['num_source'] == 1:
			...
			if self.return_fname:
				return mic_signals, gts, os.path.basename(sig_path)
			else:
...
class LocataDataset(Dataset):
	...
	"""
	def __init__(self, paths, array, fs=16000, tasks=(1,3,5), recording=None, dev=False, transforms = None, mic_idx=[8,5], return_acoustic_scene=False):
		"""
	...
	def __getitem__(self, idx):
		...
		if self.mic_idx:
			mic_signals = mic_signals[:, self.mic_idx]
...
class Segmenting_SRPDNN(object):
	...
	def __call__(self, x, acoustic_scene):
		...
		L = x.shape[0]   # n channel 신호 (72000,2) -> L은 sample 수를 의미
		N_w = np.floor(L/self.step - self.K/self.step + 1).astype(int)

		acoustic_scene.fs = 16000
